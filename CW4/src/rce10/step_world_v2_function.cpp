			...
			for(unsigned t=0;t<n;t++){
				for(uint32_t y=0;y<h;y++){
					for(uint32_t x=0;x<w;x++){
						hpce::rce10::kernel_xy(x, y, w, inner, outer, &world.state[0], (const uint32_t*) &world.properties[0], &buffer[0]);
					}  // end of for(x...
				} // end of for(y...
...
int main(int argc, char *argv[])
{
	...
	try{
		...
		hpce::rce10::StepWorldV2Function(world, dt, n);
